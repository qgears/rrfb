 ...
 * Decode a QOI file given as an ArrayBuffer.
     ...
     Modified by https://github.com/rizsi : longer runs are also handled. Original source from: https://github.com/kchapelier/qoijs where the repo states it is MIT licensed
 *
 ...
 */
function decodeAdd (arrayBuffer, byteOffset, byteLength, outputChannels, resultToAddTo) {
    ...
    // Modded by Rizsi
    if (magic1 !== 0x71 || magic2 !== 0x6F || magic3 !== 0x69 || magic4 !== 0x67) {
...
//    const result = new Uint8Array(pixelLength);
    ...
    for (; pixelPosition < pixelLength && arrayPosition < byteLength - 4; pixelPosition += outputChannels) {
        if (run > 0) {
            ...
            {
            	pixelPosition+=outputChannels*run;
            	run=0;
            }
		...
		//if(red!=0 || green!=0 || blue!=0 || alpha!=0) this optimization is not useful because in case of runs we filter for 0
		{
        if (outputChannels === 4) { // RGBA
            resultToAddTo[pixelPosition] -= red;
            resultToAddTo[pixelPosition + 1] -= green;
            resultToAddTo[pixelPosition + 2] -= blue;
            resultToAddTo[pixelPosition + 3] -= alpha;
        } else { // RGB
            resultToAddTo[pixelPosition] -= red;
            resultToAddTo[pixelPosition + 1] -= green;
            resultToAddTo[pixelPosition + 2] -= blue;

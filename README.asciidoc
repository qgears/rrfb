...
----
 ...
 $ java -jar rrfb.jar --connectCommand rrfb --host 127.0.0.1

----

Starts a web server on 0.0.0.0:9012 where RRFB is accessible. When a client connects then the rrfb program is started that will connect the client to the X desktop.

If --host 127.0.0.1 is set then the web server will only be accessible from the localhost.
